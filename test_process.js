var document = {
    querySelectorAll: function(selector) {
        if (selector === '.p-input') {
            var inputs = [];
            for(var i=0; i<8; i++) inputs.push({value: 'P'+i});
            return inputs;
        }
        if (selector === '.seed-toggle') {
            var toggles = [];
            for(var i=0; i<8; i++) toggles.push({classList: {contains: function(){return false;}}});
            return toggles;
        }
        if (selector === '.screen') return [{classList: {remove: function(){}}}];
        return [];
    },
...
var window = { location: { search: '' }, origin: '', pathname: '', scrollTo: function(){} };
...
App.state.format = 'individual';
App.state.numPlayers = 8;
App.state.numCourts = 2;

try {
    console.log("Processing...");
    App.processPlayers();
    console.log("Success! Active Screen: " + App.state.tournamentId);
} catch(e) {
